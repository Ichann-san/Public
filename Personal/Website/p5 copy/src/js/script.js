document.addEventListener('DOMContentLoaded', () => {
    ...
    const courses = [
        ...
        },
        {
            ...
            img: "https://placehold.co/600x400/636f1f/ffffff?text=Algorithms",
    ...
    const desktopLoginBtnContainer = document.getElementById('desktop-login-btn-container'); // Get the new container
    ...
    function renderNavLinks() {
        if (!desktopNavContainer || !mobileNavContainer || !desktopLoginBtnContainer) return;
        ...
        // REFINED: Render login button dynamically for both desktop and mobile
        const loginButtonHTML = `<button class="bg-indigo-600 text-white px-4 py-2 rounded-lg hover:bg-indigo-700 transition-transform hover:scale-105">Login</button>`;
        desktopLoginBtnContainer.innerHTML = loginButtonHTML;
        mobileNavContainer.innerHTML += `<div class="mt-6">${loginButtonHTML}</div>`;
    }
    ...
    if (hamburgerBtn && mobileMenu && closeMenuBtn) {
        ...
        mobileMenu.addEventListener('click', (e) => {
            if (e.target.tagName === 'A' || e.target.tagName === 'BUTTON' || e.target === mobileMenu) {
