...
def start_http_server():
    ...
    try:
        while True:
            ...
            request_file = request_parts[1] if len(request_parts) > 1 else ""
            accept_encoding = ""
            for line in request_lines:
                if line.lower().startswith("accept-encoding:"):
                    accept_encoding = line.split(":", 1)[1].strip()
                    break
            print(f"Request header: {request_lines}")
            ...
            if request_file == '/status':
                ...
                if "deflate" in accept_encoding.lower():
                    body = zlib.compress(json_data)
                    response_header = (
                        "HTTP/1.1 200 OK\r\n"
