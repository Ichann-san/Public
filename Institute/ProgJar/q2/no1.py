...
class Message:
    ...
    def deserialize(serialized_message):
        ...
        if isinstance(serialized_message, bytes):
            xml_text = serialized_message.decode("utf-8")
        else:
            xml_text = serialized_message

        root = ET.fromstring(xml_text)
        username = root.findtext("username", default="")
        text = root.findtext("text", default="")
        timestamp_text = root.findtext("timestamp", default="")
        try:
    ...
    def serialize(self):
        ...
        ET.SubElement(root, "username").text = str(self.username)
        ET.SubElement(root, "text").text = str(self.text)
        if isinstance(self.timestamp, datetime):
            timestamp_text = self.timestamp.isoformat()
        else:
            timestamp_text = str(self.timestamp)
        ET.SubElement(root, "timestamp").text = timestamp_text
...
def main():
    ...
    logger.info("Server listening on localhost:12345")
