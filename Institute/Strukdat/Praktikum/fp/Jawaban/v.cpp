...
public:
    ...
    int nodedepth(int val) { return nodedepth(root, val, 1); }
};
...
void solve(int n, tree &pohon) {
    ...
    while(n--) {
        ...
        if (s == "INSERT") {
            cin >> x;
            if(!pohon.search(x)) pohon.insert(x);
        }
        if (s == "DELETE") {
            cin >> x;
            if(pohon.search(x)) pohon.remove(x);
        }
        if (s == "SEARCH") {
            cin >> x;
            if(pohon.search(x)) cout << pohon.nodedepth(x) << endl;
            else cout << -1 << endl;
        }
        if (s == "PRINT") {
            if(pohon.exists()) {
