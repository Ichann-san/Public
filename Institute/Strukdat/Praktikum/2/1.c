...
TreeNode* insert_node(TreeNode* node, int data) {//avltree
...
TreeNode* delete_node(TreeNode* node, int data) {//avltree
    if (!node) return NULL;
    if (data < node->data)
...
TreeNode* search_node_path(TreeNode* node, int data) {
    printf("%d ", node->data);
    if (!node || node->data == data) return node;
    if (data < node->data) return search_node_path(node->left, data);
    return search_node_path(node->right, data);
}
...
int main() {
    ...
    for(int i = 0; i < n; i++) {
        ...
        if(strcmp(s, "INSERT") == 0) {
            ...
            if (search_node(tree.root, x)) {
                printf("%d found with path: ", x);
                TreeNode* node = search_node_path(tree.root, x);
                printf("\n");
            } else {
                printf("%d not found\n", x);
            }
