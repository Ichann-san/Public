...
void tambah() {
    ...
    if (!fdat) { if(!fdat) file_error(data_file); return; }

    Barang component;
    Indeks index;
    printf("Code: "); scanf("%s", component.kode);
    getchar();
    printf("Component name: "); fgets(component.nama, nama_length, stdin);
    printf("Stock: "); scanf("%d", &component.stok);
    printf("Price: "); scanf("%lf", &component.harga);
    component.nama[strcspn(component.nama, "\n")] = '\0'; // \n -> \0
    int len = strlen(component.nama);
    for (int i = len; i < nama_length - 1; i++) { component.nama[i] = ' '; }
...
void ubah() {
    ...
    printf("Enter code to modify: "); scanf("%s", cari);
    load_index();
    FILE *fdat = fopen(data_file, "rb+");
    if (!fdat) { file_error(data_file); return; }
    ...
    for (int i = 0; i < komponen_jumlah; i++) {
        if (strcmp(komponen_indeks[i].kode, cari) == 0) {
            ...
            getchar();
            printf("New data\n");
            printf("Component name: "); fgets(komponen.nama, nama_length, stdin);
...
void hapus() {
    char cari[kode_length];
    printf("Enter code to delete: "); scanf("%s", cari);
    load_index();
    FILE *fdat = fopen(data_file, "rb");
    ...
    if (!fdat) { if(!fdat) file_error(data_file); return; }
    Barang component;
    bool found = false; long offset = 0;
    for (int i = 0; i < komponen_jumlah; i++) {
        fseek(fdat, komponen_indeks[i].offset, SEEK_SET);
        fread(&component, sizeof(Barang), 1, fdat);
    ...
    fclose(fdat);  fclose(ftemp); fclose(fnew_index);
    remove(data_file); rename("../data/temp.dat", data_file);
    remove(data_indeks); rename("../data/newindex.dat", data_indeks);
    if (found) printf("Data deleted\n");
...
void menu() {
    ...
    while (1) {
        ...
        if (ch == 224) {
            int arah = _getch();
            if(temp >= '1' && temp <= '6') {
                if(arah == 72) temp = (temp >= '6') ? '1' : ++temp; // arrow atas
                if(arah == 80) temp = (temp <= '1') ? '6' : --temp; // arrow bawah
            }
            input = temp;
        } else if(ch == 13) { // enter
            switch (input) {
                ...
                case '3': case 'X' : view_indeks(); break;
                case '4': case 'T' : noindeks(); break;
                case '5': case 'H' : hapus(); break;
